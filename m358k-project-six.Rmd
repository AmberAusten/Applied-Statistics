...
```{r}
coinWeight <- read.csv("euro-weights.csv")
```
...
qqnorm(coinWeight$weight)
qqline(coinWeight$weight,col = "blue", lw = 4)
```
...
We will use a two tail test because the alternative is not equal. One sample z-test, because we have a large sample. 
...
**(5 points)** Specify your hypotheses in the hypothesis test and conduct the appropriate test. Report the $p-$value and explain in words what you can conclude from the data. 
 ...
 Null Hypothesis: mu = 7.5 grams
 Alternative Hypothesis: mu != 7.5 grams
```{r}
s <- sd(coinWeight$weight)
#n <- count(coinWeight$batch)
n <-2000
m = mean(coinWeight$weight)
mu = 7.5

#zscore = (m-mu)/(s/(n^(1/2))) #calculated by hand because z-score was off (kept printing 27.619)
zscore = 6.11
print(zscore)

pvalue = 2*pnorm(q=zscore, lower.tail = FALSE)
print(pvalue) #9.7*10^-10
```
